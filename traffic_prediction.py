"""
Animated demo using traffic prediction data for Online SVR. 2 features
generated at 100 random samples, sent through a sine function, and used to
predict and learn.

Usage:
  python3 traffic_prediction.py <C> <epsilon> <kernel parameter> <log filename>

C is the regularizatin parameter, essentially defining the limit on how close
the learner must adhere to the dataset (smoothness). Epsilon is the acceptable
error, and defines the width of what is sometimes called the "SVR tube".
The kernel parameter is the scaling factor for comparing feature distance
...
def animate(i):
    ...
    if i < testSetX.shape[0] - 1:
        print('Iteration prediction error was {0:.4f}'.format(
            np.abs(testSetY[i + 1] - ydata[i]).item()))
    print('@@@@@@ Iteration elapsed time: {0:.3f} seconds @@@@@@'.format(
        elapsed))
...
plt.title(
    'C={0},eps={1},kernelParam={2}\n{3}'.format(C, eps, kernelParam, filename))
plt.legend(loc='upper left')

# Set up learner
OSVR = online_svr.OnlineSVR(
