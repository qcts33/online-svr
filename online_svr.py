#!/usr/bin/env python3
"""Implementation of Online Support Vector Regression (OSVR)
as library for a class project in 16-831
...
Parameters defined in main() below. C is the regularization parameter,
essentially defining the limit on how close the learner must adhere to
the dataset (smoothness). Epsilon is the acceptable error, and defines
the width of what is sometimes called the "SVR tube". The kernel parameter
is the scaling factor for comparing feature distance (this implementation
...
def sign(x):
    """ Returns sign. Numpys sign function returns 0 instead of 1 for zero
    values. :(
    """
    if x >= 0:
        return 1
...
class OnlineSVR:
    ...
    def findMinVariation(self, H, beta, gamma, i):
        ...
        # Check for duplicate minimum values, grab one with max gamma/beta,
        # set others to inf
        # Support set
        if Ls.size > 1:
            minS = np.abs(Ls).min()
            results = np.array(
                [k for k, val in enumerate(Ls) if np.abs(val) == minS])
            if len(results) > 1:
        ...
        if Le.size > 1:
            minE = np.abs(Le).min()
            results = np.array(
                [k for k, val in enumerate(Le) if np.abs(val) == minE])
            if len(results) > 1:
        ...
        if Lr.size > 1:
            minR = np.abs(Lr).min()
            results = np.array(
                [k for k, val in enumerate(Lr) if np.abs(val) == minR])
            if len(results) > 1:
        ...
        # Find minimum absolute variation of all, retain signs. Flag determines
        # set-switching cases.
    ...
    def computeBetaGamma(self, i):
        ...
        if len(self.supportSetIndices
               ) == 0 or Qxi.size == 0 or Qxs.size == 0 or beta.size == 0:
            gamma = np.array(np.ones_like(Qxi))
        else:
            gamma = Qxi + np.append(
                np.ones([self.numSamplesTrained, 1]), Qxs, 1) @ beta
    ...
    def adjustSets(self, H, beta, gamma, i, flag, minIndex):
        ...
        # move sample from Support set to Error or Remainder set
            ...
            # move from support to error set
                ...
                print('Moving sample {0} from support to error set.'.format(
                    index))
                self.errorSetIndices.append(index)
                self.R = self.removeSampleFromR(minIndex)
                self.supportSetIndices.pop(minIndex)
            else:
                print('Issue with set swapping, flag 2.', 'weightsValue:',
                      weightsValue)
                sys.exit()
    ...
    def learn(self, newSampleX, newSampleY):
        ...
        while not newSampleAdded:
            ...
            # Find minimum variation and determine how we should shift samples
            # between sets
            ...
            H, newSampleAdded = self.adjustSets(H, beta, gamma, i, flag,
                                                minIndex)
        if self.debug:
            print('weights', self.weights)
